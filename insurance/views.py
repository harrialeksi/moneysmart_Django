...
from .models import Insurance, InsuranceUsp, Provider
...
def insurances(request):
    ...
    for insurance in insurances:
        ...
        row.promotion = insurance['promotion']
        row.keyfeatures = insurance['keyFeatures']
        row.annualinterest = insurance['annualInterest']
        row.incomeequirement = insurance['incomeRequirement']
        row.insuranceassociation = insurance['insuranceAssociation']
        row.wirelesspayment = insurance['wirelessPayment']
        row.save()

        for usp in insurance['usp']:
            car_usp = InsuranceUsp.objects.create(dd=usp['ratio'], dt=usp['text'], insurance_id=row.id)
            car_usp.save()
    ...
    return render(request, "cards.html", {"cards": queryset, "providers": providers, "number": number})
